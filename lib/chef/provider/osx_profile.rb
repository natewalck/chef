  ...
  class Provider
    class OsxProfile < Chef::Provider
      ...
      def load_current_resource
        ...
        @current_resource.installed(profile_installed?)
      end
      ...
      def action_install
        unless @current_resource.installed
          converge_by("install profile #{@new_profile_identifier}") do
            ...
            get_installed_profiles(update=true)
          end
        end
      end
      ...
      def load_profile_hash(new_profile)
          ...
          unless new_profile.end_with?('.mobileconfig')
            error_string = "'#{new_profile}' is not a valid profile"
          ...
          profile = read_plist(cookbook_profile)
          profile
      end
      ...
      def get_profile_hash(new_profile)
        if @new_resource.action.include?(:install)
          if new_profile.is_a?(Hash)
            ...
            profile = load_profile_hash(new_profile)
          else
            profile = nil
          end
        end
      ...
      def read_plist(xml_file)
        installed_profiles = Plist::parse_xml(xml_file)
      end
